...
extension APIManager {
    ...
    func retrivePokemon(pokeID: Int, completionHandler: (_ pokemon: Pokemon) -> Void) {
    Alamofire.request(.GET, "\(APIManager.sharedInstance.kBasePokemonURL)\(pokeID)", parameters: nil)
        .responseJSON(options: JSONSerialization.ReadingOptions.AllowFragments) { response in
